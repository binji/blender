...
static void update_node_vb(PBVH *bvh, PBVHNode *node)
{
	...
	if (node->flag & PBVH_Leaf) {
		...
		/*if (!(node->vb.bmin[0] < node->vb.bmax[0] &&
			  node->vb.bmin[1] < node->vb.bmax[1])) {
			BLI_assert(0);
			}*/
	}
	else {
                                ...
                                int totnode, float (*face_nors)[3])
{
	...
	if (bvh->type == PBVH_BMESH) {
		...
		//BM_mesh_normals_update(bvh->bm, TRUE);
		return;
	}
                          ...
                          float *dist)
{
	...
	switch (bvh->type) {
		...
		case PBVH_GRIDS:
			...
			break;
		case PBVH_BMESH:
			hit = pbvh_bmesh_node_raycast(node, ray_start, ray_normal, dist, !!origco);
			break;
	}
